...
class __Info:
    ...
    def clusters_info(self):
        ...
        return self._create_board(['clusters','radius'])
    ...
    def general_info(self, **args):
        ...
        if not hide_nan:
            ...
            for name,code in self._nan_general_index().items():
                print(f"{name:<25}-{'':<10}{code}")
        return self._create_board(['general', 'score_index_GDI']).rename(columns={0:'General Informations'})
